import { IconAlignBoxLeftStretch, IconBook2, IconBrandFacebook, IconBrandInstagram, IconBrandTiktok, IconBrandWhatsapp, IconBrandX, IconBrandYoutube, IconMusic, IconPhoto, IconShoppingBag } from "@tabler/icons-react"
...
export function NoCodingRequired(props: Props) {
  ...
  return (
    <div className={twMerge("relative flex flex-col items-end pt-16", className)} {...rest}>
      <div className="w-2/5 flex flex-col items-stretch relative mr-32">
        ...
        <div className="flex flex-col justify-end items-stretch absolute top-0 left-0 w-full h-full gap-4 pb-12">
          <InlineCard className="-translate-x-[16rem] rotate-[6deg] flex flex-col items-stretch gap-4">
            <div className="grid grid-cols-6 gap-4">
              <div className="flex flex-col items-stretch">
                <IconAlignBoxLeftStretch size={24} className="text-purple-600" />
              </div>
              <div className="flex flex-col items-stretch gap-2 col-span-5">
                <Title>Join My Waitlist</Title>
                <Subtitle>
                  I`m taking orders right now for my upcoming product so hurry before the waitlist is full.
                  I`ll be providing updates as we near the launch date
                </Subtitle>
              </div>
            </div>
          ...
          <InlineCard className="-translate-x-[10rem] rotate-[4deg]" Graphic={<IconShoppingBag size={16} className="text-green-600" />} title="Shop for merch" />
          <InlineCard className="-translate-x-[5rem] rotate-[2deg]" Graphic={<IconMusic size={16} className="text-red-600" />} title="Listen to my new single" />
          <InlineCard className="-translate-x-[2.5rem]" Graphic={<IconBook2 size={16} className="text-stone-800" />} title="Check out my new course" />
        </div>
      </div>
...
function InlineCard(props: InlineCardProps) {
  ...
  return (
    <div className={twMerge('bg-white/40 backdrop-blur-xl flex flex-row items-center gap-4 rounded-xl px-4 py-3 shadow-xl border border-stone-200', className)} {...rest}>
      {!!children && (<>{children}</>)}
      {!children && (
        <>
          {Graphic}
...
function Title({ children }: { children: string | undefined }) {
  return (
    <span className="text-stone-800 text-xs">{children}</span>
  )
}

function Subtitle({ children }: { children: string | undefined }) {
  return (
    <span className="text-stone-600 text-xs font-light text-balance">{children}</span>
  )
}