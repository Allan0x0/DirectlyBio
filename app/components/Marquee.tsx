import { IconClockHour4Filled, IconLayout2Filled, IconTool } from '@tabler/icons-react';
...
export function Marquee(props: ComponentProps<'div'>) {
  ...
  return (
    <div className={twMerge("overflow-hidden flex flex-col items-stretch bg-orange-200 border-b border-stone-800", className)} {...rest}>
      <div className="flex flex-row items-center gap-6 animate-marquee min-w-full whitespace-nowrap">
        {children}
      </div>
    </div>
  )
...
export function MarqueeItem(props: ItemProps) {
  ...
  return (
    <div className={twMerge("flex flex-row items-center gap-4 p-6", className)} {...rest}>
      <div className="flex flex-col justify-center items-center">
        {Icon}
      </div>
      <div className="flex flex-col justify-center items-center">
        <span className="uppercase text-xl tracking-wide font-bold text-stone-800">{text}</span>
      </div>
    </div>
  )
}
